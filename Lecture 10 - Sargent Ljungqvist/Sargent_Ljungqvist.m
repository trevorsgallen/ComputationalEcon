            ...
            %Solve for V
                    for w_ind = 1:length(w_space)
                        for h_ind = 1:length(h_space)
                            ...
                            h = h_space(h_ind);
                            I = w.*h;
                            %Grab the relevant UI index (highest lower than I)
                                ind2 = max(find(I<= UI_interval));
                                %Old
                    ...
                    for h_ind = 1:length(h_space)
                        ...
                        for hprime_ind = 1:length(h_space)
                                for wdraw_ind = 1:length(w_space)
                                    ...
                                    %Grab the UI index
                                        ind2prime = max(find(UI_interval<=hprime.*wdraw));
...
%                                         [temp2, ind2prime] = min(abs(UI_interval-hprime.*wdraw)+(UI_interval<hprime.*wdraw)*1000);
            ...
            %Solve for V_0
                for h_ind = 1:length(h_space)
                    ...
                    %Solve for best unemployed value, store value and
                    %policy
            ...
            %Error is maximum absolute change in any value 
            error = max([max(abs(V_0_new-V_0)),max(max(abs(V_b_new-V_b))),max(max(abs(V_new-V)))]);

            %Display count and means
            format long g
            sprintf(' %5g %0.5g %3g %3g %3g ',zcount,error,mean(mean(V_new)),mean(mean(V_b_new)),mean(V_0_new))

            %New (LHS) values are now Old (RHS) values
            V_0 = V_0_new;
            V_b = V_b_new;
            V = V_new;
        save sol.mat V_0 V_b V V_0_LF V_b_LF V_LF;
    end
...
%% Laissez-Faire economy
    ...
    while error > 1e-10
        zcount = zcount+1;
            ...
            [zcount,error,mean(mean(V_new_LF)),mean(mean(V_b_new_LF)),mean(V_0_new_LF)]
